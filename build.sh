...
# Install pre-push hook to prevent pushing to the wrong remote, only when this
# is a git repository
if [ -d '.git' ]; then
  PRE_PUSH_HOOK_SOURCE='tools/pre-push'
  PRE_PUSH_HOOK_DESTINATION='.git/hooks/pre-push'
  if [ ! -x "$PRE_PUSH_HOOK_DESTINATION" ] \
      || ! diff "$PRE_PUSH_HOOK_DESTINATION" "$PRE_PUSH_HOOK_SOURCE" >/dev/null
  then
