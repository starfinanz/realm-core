...
public:
    ...
    RowExpr get(RowKey row_key);
    ConstRowExpr get(RowKey row_key) const;
    ...
    RowExpr operator[](RowKey row_key);
    ConstRowExpr operator[](RowKey row_key) const;
    ...
    // a) only works with num_rows==1
    // b) give 0 for row_key for autogenerated key,
    //    use non-0 in order to supply a key.
    // throws if key is already in use.
    std::unique_ptr<IntegerColumn> add_empty_rows(size_t num_rows);
    RowKey insert_empty_row(RowKey row_key = 0, size_t num_rows = 1);
    ...
    int64_t     get_int(size_t column_ndx, RowKey row_key) const;
    bool        get_bool(size_t column_ndx, RowKey row_key) const;
    OldDateTime get_olddatetime(size_t column_ndx, RowKey row_key) const;
    float       get_float(size_t column_ndx, RowKey row_key) const;
    double      get_double(size_t column_ndx, RowKey row_key) const;
    StringData  get_string(size_t column_ndx, RowKey row_key) const;
    BinaryData  get_binary(size_t column_ndx, RowKey row_key) const;
    Mixed       get_mixed(size_t column_ndx, RowKey row_key) const;
    DataType    get_mixed_type(size_t column_ndx, RowKey row_key) const;
    Timestamp   get_timestamp(size_t column_ndx, RowKey row_key) const;

    template<class T> T get(size_t c, RowKey r) const;

    size_t get_link(size_t column_ndx, RowKey row_key) const;
    bool is_null_link(size_t column_ndx, RowKey row_key) const;
    ...
    size_t get_link_count(size_t column_ndx, RowKey row_key) const;
    bool linklist_is_empty(size_t column_ndx, RowKey row_key) const;
    bool is_null(size_t column_ndx, RowKey row_key) const;
    ...
    size_t get_subtable_size(size_t column_ndx, RowKey row_key) const;
...
private:
    ...
    size_t get_num_strong_backlinks(RowKey row_key) const;
    ...
    Table* get_subtable_accessor(size_t col_ndx, RowKey row_key);
    ...
    void discard_subtable_accessor(size_t col_ndx, RowKey row_key);
...
inline Table::RowExpr Table::get(RowKey row_key)
{
    return RowExpr(this, row_key);
}

inline Table::ConstRowExpr Table::get(RowKey row_key) const
{
...
inline Table::RowExpr Table::operator[](RowKey row_key)
{
    return get(row_key);
}

inline Table::ConstRowExpr Table::operator[](RowKey row_key) const
{
    return get(row_key);
}
...
{
    return insert_empty_row(0, num_rows); // Throws
}

inline std::unique_ptr<IntegerColumn> Table::add_empty_rows(size_t num_rows)
{
    ref_type ref = IntegerColumn::create(Allocator::get_default());
    auto res = std::unique_ptr<IntegerColumn>(new IntegerColumn(Allocator::get_default(), ref));
    for (size_t i=0; i<num_rows; ++i) {
        RowKey r = insert_empty_row(0,1);
        res->add(r);
    }
    return res;
}
...
inline bool Table::is_null_link(size_t col_ndx, RowKey row_key) const
{
